  ...
  <script type="module">
    ...
    async function carregarProdutos(tipo) {
      try {
        ...
        return docSnap.exists() ? docSnap.data().produtos || [] : [];
      } catch (error) {
    ...
    async function salvarProdutos(produtos, tipo) {
      try {
        await setDoc(doc(db, "estoque", `produtos_${tipo}`), { produtos });
      } catch (error) {
    ...
    function observarMudancas(tipo, callback) {
      ...
      return onSnapshot(docRef, (docSnap) => {
        const produtos = docSnap.exists() ? docSnap.data().produtos || [] : [];
        callback(produtos);
      });
    }
...
<div class="app">
  ...
  <header>
    <div class="title">
      ...
      <span>🧸 SISTEMA DE ESTOQUE - MOLLIBEAR 🐻</span>
    </div>
    ...
    <div class="actions">
      ...
      <button class="button-fofo">❓ Sobre</button>
      <button id="loginBtn" class="button-fofo" style="display: none;">🔑 Login</button>
      <button id="logoutBtn" class="button-fofo" style="display: none;">🚪 Sair</button>
    </div>
  </header>
  ...
  <div class="content">
    ...
    <aside id="asideCadastro">
      ...
      <div class="card">
        ...
        <input type="number" id="quantidadeProduto" class="input-fofo">
      ...
      <div class="card">
        ...
        <input type="text" id="buscaProduto" class="input-fofo" placeholder="Busque por código, descrição, cor, modelo ou estante">
    ...
    <main>
      ...
      <div id="conteudoDinamico">
        ...
        <div id="mapaContainer" style="display: block;">
          ...
          <div class="legenda-mapa">
            <div class="legenda-item"><span class="cor-verde"></span> 0-50% ocupado</div>
            <div class="legenda-item"><span class="cor-amarelo"></span> 50-80% ocupado</div>
            <div class="legenda-item"><span class="cor-vermelho"></span> 80-100% ocupado</div>
      ...
      <div id="produtosCadastradosModal" class="modal" style="display: none;">
        <div class="modal-content">
          ...
          <div style="display: flex; gap: 10px; margin-bottom: 15px;">
            <input type="text" id="buscaProdutosModal" class="input-fofo" placeholder="Busque por código, descrição, cor, modelo ou estante" style="flex: 1;">
      ...
      <div id="movimentacaoModal" class="modal" style="display: none;">
        <div class="modal-content" style="width: 90%; max-width: 500px;">
          <span class="close-movimentacao">&times;</span>
          <h2 id="tituloMovimentacao">📦 Movimentação de Estoque</h2>
          <div id="infoProdutoMovimentacao" style="margin-bottom: 20px;"></div>
          <div style="display: flex; gap: 10px; margin-bottom: 20px;">
            <button id="entradaBtn" class="button-fofo" style="flex: 1; background-color: #d4edda; color: #155724;">➕ Entrada</button>
            <button id="saidaBtn" class="button-fofo" style="flex: 1; background-color: #f8d7da; color: #721c24;">➖ Saída</button>
          </div>
          <div id="formMovimentacao" style="display: none;">
            <label>Quantidade</label>
            <input type="number" id="quantidadeMovimentacao" class="input-fofo" min="1" value="1">
            <button id="confirmarMovimentacaoBtn" class="save button-fofo" style="margin-top: 15px; width: 100%;">✅ Confirmar</button>
          </div>
        </div>
      </div>
      ...
      <div id="loginModal" class="modal" style="display: none;">
        <div class="modal-content" style="width: 90%; max-width: 400px;">
          ...
          <input type="password" id="senhaInput" class="input-fofo" placeholder="Digite a senha" style="margin-top: 6px;">
...
function calcularPorcentagemOcupacao(estanteNumero) {
  ...
  if (produtosEstante.length === 0) return 0;
  
  const totalItens = produtosEstante.reduce((sum, p) => sum + parseInt(p.quantidade || 0), 0);
  // Supondo que cada estante tem capacidade para 100 itens (você pode ajustar)
  const capacidade = 100;
  return (totalItens / capacidade) * 100;
}
...
function getCorEstante(estanteNumero) {
  ...
  if (porcentagem === 0) return "#f5e6d3"; // Vazia
  if (porcentagem <= 50) return "#d4edda"; // Verde (0-50%)
  if (porcentagem <= 80) return "#fff3cd"; // Amarelo (50-80%)
  return "#f8d7da"; // Vermelho (80-100%)
}
...
function gerarCardProduto(produto, mostrarEstanteNivel = true, mostrarCheckbox = false, index = null) {
  const corEstante = mostrarEstanteNivel && produto.estante ? getCorEstante(produto.estante) : null;
  ...
  return `
    <div style="margin-bottom: 15px; padding: 12px; border: 2px solid #f5e6d3; border-radius: 8px; background-color: #fffaf0; display: flex; justify-content: space-between; align-items: center; flex-direction: column; position: relative;">
      ...
      <div style="flex: 1; width: 100%;">
        ...
        <p style="margin: 5px 0; font-size: 16px;"><strong>📊 Quantidade:</strong> ${produto.quantidade}</p>
      ...
      ${mostrarEstanteNivel ? `
        <div style="display: flex; gap: 8px; justify-content: center; margin-top: 10px;">
          <button class="button-movimentacao entrada" onclick="abrirModalMovimentacao('${produto.codigo}', '${produto.cor || ''}', '${produto.modelo}', ${produto.quantidade}, 'entrada')">➕ Entrada</button>
          <button class="button-movimentacao saida" onclick="abrirModalMovimentacao('${produto.codigo}', '${produto.cor || ''}', '${produto.modelo}', ${produto.quantidade}, 'saida')">➖ Saída</button>
        </div>
      ` : ''}
    </div>
  `;
...
async function exibirListaEstoque(pagina = 1) {
  ...
  if (produtosPaginados.length === 0) {
    ...
    produtosPaginados.forEach((produto, index) => {
      listaEstoque.innerHTML += gerarCardProduto(produto, false);
    });
  }
...
function abrirModalMovimentacao(codigo, cor, modelo, quantidadeAtual, tipo) {
  ...
  modal.dataset.modelo = modelo;
  modal.dataset.quantidadeAtual = quantidadeAtual;
  modal.dataset.tipo = tipo;
  
  // Exibir informações do produto
  infoProduto.innerHTML = `
    <p><strong>Código:</strong> ${codigo}</p>
    <p><strong>Descrição:</strong> ${produtosAtuais.find(p => p.codigo === codigo && p.cor === cor && p.modelo === modelo)?.descricao || 'N/A'}</p>
    ${cor ? `<p><strong>Cor:</strong> ${cor}</p>` : ''}
    <p><strong>Modelo:</strong> ${modelo}</p>
    <p><strong>Quantidade Atual:</strong> ${quantidadeAtual}</p>
    <p><strong>Tipo:</strong> ${tipo === 'entrada' ? '➕ Entrada' : '➖ Saída'}</p>
  `;
  
  // Exibir formulário
  formMovimentacao.style.display = 'block';
  document.getElementById('quantidadeMovimentacao').value = '1';
  document.getElementById('quantidadeMovimentacao').focus();
...
// Função para confirmar movimentação
async function confirmarMovimentacao() {
  if (!estaLogado()) {
    alert('❌ Você não tem permissão para fazer movimentações!');
    return;
  }
  
  const modal = document.getElementById('movimentacaoModal');
  const codigo = modal.dataset.codigo;
  const cor = modal.dataset.cor;
  const modelo = modal.dataset.modelo;
  const quantidadeAtual = parseInt(modal.dataset.quantidadeAtual);
  ...
  if (tipo === 'entrada') {
    ...
    if (quantidadeMov > quantidadeAtual) {
      alert('❌ Quantidade de saída não pode ser maior que a quantidade atual!');
      return;
    }
    novaQuantidade = quantidadeAtual - quantidadeMov;
  }
  ...
  const produtoIndex = produtos.findIndex(p => p.codigo === codigo && p.cor === cor && p.modelo === modelo);
  
  if (produtoIndex !== -1) {
    // Registrar no histórico (simplificado - depois implementamos o histórico completo)
    const historico = {
      data: new Date().toISOString().split('T')[0],
      codigo: codigo,
      descricao: produtos[produtoIndex].descricao,
      cor: cor,
      modelo: modelo,
      quantidadeAnterior: quantidadeAtual,
      quantidadeNova: novaQuantidade,
      tipo: tipo === 'entrada' ? 'Entrada' : 'Saída',
      quantidadeMovimentada: quantidadeMov,
      usuario: "Administrador"
    };
    
    // Salvar no Firestore (simplificado - depois implementamos a coleção de histórico)
    console.log("Movimentação registrada:", historico);
    
    // Atualizar quantidade
    produtos[produtoIndex].quantidade = novaQuantidade;
    await window.salvarProdutos(produtos, modoAtual);
    
    alert(`✅ Movimentação realizada! Nova quantidade: ${novaQuantidade}`);
...
async function filtrarProdutosModal(pagina = 1) {
  ...
  if (produtosPaginados.length === 0) {
    ...
    produtosPaginados.forEach((produto, index) => {
      ...
      listaProdutos.innerHTML += gerarCardProduto(produto, false, true, globalIndex);
    });
  }
...
async function excluirSelecionados() {
  ...
  if (confirm(`🐻 Tem certeza que deseja excluir ${produtosSelecionados.length} produtos selecionados?`)) {
    ...
    await window.salvarProdutos(novosProdutos, modoAtual);
...
async function buscarProduto() {
  ...
  if (produtosEncontrados.length === 0) {
    ...
    produtosEncontrados.forEach(produto => {
      resultadoBusca.innerHTML += gerarCardProduto(produto, false);
    });
  }
}
...
const estantes = [
  ...
  {n:17, x:1030, y:700, w:100, h:40},
  {n:18, x:920, y:700, w:100, h:40},
  {n:19, x:810, y:700, w:100, h:40},
  {n:20, x:700, y:700, w:100, h:40},
  {n:21, x:590, y:700, w:100, h:40},
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Inicializar o SVG (será preenchido pela função atualizarCoresEstantes)
  atualizarCoresEstantes();
  ...
  document.querySelector('.close-movimentacao').addEventListener('click', fecharModalMovimentacao);
  ...
  document.getElementById('entradaBtn').addEventListener('click', () => {
    ...
    document.getElementById('tituloMovimentacao').textContent = '➕ Entrada de Estoque';
  });
  document.getElementById('saidaBtn').addEventListener('click', () => {
    document.getElementById('formMovimentacao').style.display = 'block';
    document.getElementById('tituloMovimentacao').textContent = '➖ Saída de Estoque';
  });
...
<style>
  ...
  .button-movimentacao.entrada {
    background-color: var(--verde);
    color: #155724;
  }

  .button-movimentacao.saida {
    background-color: var(--vermelho);
    color: #721c24;
  }

  .button-movimentacao:hover {
    transform: scale(1.05);
    opacity: 0.9;
  }
  ...
  .close, .close-estante, .close-login, .close-movimentacao {
    float: right;
    font-size: 28px;
    cursor: pointer;
    color: var(--bege-escuro);
  ...
  .close:hover, .close-estante:hover, .close-login:hover, .close-movimentacao:hover {
    color: #ff6b6b;
  }
